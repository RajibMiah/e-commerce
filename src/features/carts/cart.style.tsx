...
const PopupItemCount = styled.div`
  display: inline-flex;
  align-items: center;
  color: ${themeGet('colors.secondary.dark', '#000000FF')};
...
const CheckoutCartSlidePopup = styled.div`
  width: 30%;
  height: 89vh;
  ...
  right: -450px;
  z-index: 10;
  box-shadow: 0 21px 36px rgb(0 0 0 / 16%);
...
const OfferContainer = styled(({...otherProps}) => (
    <Box
        {...otherProps}
  ...
  background-color: rgb(105 99 99 / 36%);;
  cursor: pointer;
`;

const PriceContainer = styled(({...otherProps}) => (
    <Paper
        {...otherProps}
    /> //classes=
))`
...
const MuiTypo = styled(({children, otherProps}) => (
    <Typography
        {...otherProps}
        style={{
            textAlign: 'center',
