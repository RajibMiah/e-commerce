...
export const ProductCard = ({ data }: Props) => {
  const { title, image, price, salePrice, slug, discountInPercent, currency } = data;
  return (
    <Link href="/products/[slug]" as={`/products/${slug}`}>
      <a>
        <Card>
          ...
          <Box padding={20}>
            <PriceWrapper>
              <Price>{currency}{salePrice ? salePrice : price}</Price>
              {discountInPercent ? <SalePrice>{currency}{price}</SalePrice> : null}
            </PriceWrapper>
            <Title>{title}</Title>
          </Box>
        </Card>
